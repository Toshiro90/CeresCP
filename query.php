...
//codesub.php + additional account.php - Email Registration
DEFINE('CREATE_KEY', "INSERT INTO `registration` (`userid`, `user_pass`, `sex`, `email`, `last_ip`, `code`) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')");
DEFINE('CHECK_KEY', "SELECT * FROM `registration` WHERE `code` = '%s'");
...
DEFINE('GET_SLOT', "SELECT `char_id`, `char_num`, `name`, `countnames` FROM `char` WHERE `account_id` = '%d' ORDER BY `char_num`");
...
//top 100 pvp ladder - by KaityRoux, eathena script by Mercurial
DEFINE('TOP100PVP', "SELECT `npc_pvpsys`.`char_name`, `npc_pvpsys`.`char_id`, `npc_pvpsys`.`pvp_kills`, `npc_pvpsys`.`pvp_deaths` FROM `npc_pvpsys` ORDER BY (`pvp_kills` * 2)-`pvp_deaths` DESC LIMIT 0, 100");
...
DEFINE('Z_GET_SEX', "SELECT `sex` FROM `login` WHERE `account_id` = '%d'");
