...
if (isset($POST_opt)) {
	if ($POST_opt == 1 && isset($POST_frm_name) && !strcmp($POST_frm_name, "account")) {
		...
		$result = execute_query($query, 'account.php');
			...
			/******Needs to check email first before account is created******/
			//Generate a Key
			$thekey = rand(1111, 9999);
			$time = time();
			$thekey = "$thekey$time";
			$query = sprintf(CREATE_KEY, trim($POST_username), trim($POST_password), $POST_sex, $POST_email, $_SERVER['REMOTE_ADDR'], $thekey);
			$result = execute_query($query, 'account.php');
			if($result) {
				//Email the user
				mail($POST_email, "REGISTRATION Email: $CONFIG_name", "Hello, this is your registration email for $CONFIG_name./n Username: $POST_username/n Password: $POST_password/n Please goto: $CONFIG_domain/codesub.php?code=$thekey to complete your registration.");
			} else {
				//Looks like there was an error!
					redir("motd.php", "main_div", $lang['ACCOUNT_PROBLEM']);
				}
			//Need to make this line multi-lingual later
			redir("motd.php", "main_div", "Check your email for registration link. Be sure to check the spam folder if you can't find it!");
